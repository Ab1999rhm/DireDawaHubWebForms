...
namespace DDCH
{
    public partial class ContributorPage : System.Web.UI.Page
    {
        ...
        private void LoadDashboard()
        {
            ...
            // Load stat counts
            lblStat1Value.Text = DatabaseHelper.ExecuteScalar("SELECT COUNT(*) FROM Posts WHERE UserId = @uid", new SQLiteParameter("@uid", userId)).ToString();
            lblStat2Value.Text = DatabaseHelper.ExecuteScalar("SELECT COUNT(*) FROM Posts WHERE UserId = @uid AND Status = 'Approved'", new SQLiteParameter("@uid", userId)).ToString();
            lblStat3Value.Text = DatabaseHelper.ExecuteScalar("SELECT COUNT(*) FROM Posts WHERE UserId = @uid AND Status = 'Pending'", new SQLiteParameter("@uid", userId)).ToString();
            lblStat4Value.Text = DatabaseHelper.ExecuteScalar("SELECT COUNT(*) FROM Posts WHERE UserId = @uid AND Status = 'Deactivated'", new SQLiteParameter("@uid", userId)).ToString();
            ...
            lblNoActivity.Visible = dt.Rows.Count == 0;
