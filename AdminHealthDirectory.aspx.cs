...
namespace DDCH
{
    public partial class AdminHealthDirectory : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = DatabaseHelper.ExecuteQuery("SELECT PostId, Title, Status FROM Posts WHERE Category = @cat ORDER BY CreatedAt DESC LIMIT 3", new SQLiteParameter("@cat", "Health Directory"));
